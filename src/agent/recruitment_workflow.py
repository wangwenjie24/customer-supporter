...
def generate_recruitment_system_content():
    ...
    7.岗位职责：变量名为“job_responsibilities”，指该职位需要完成的主要工作任务和责任范围，此部分需要根据模型自身知识扩写到300字以上，如“负责家居智能照明产品研发”、“配合项目主管完成项目的监控和统筹”等。
    8.任职要求：变量名为“job_requirements”，又称“岗位要求”，是指胜任该职位所需具备的能力、技能、经验及其他素质要求，此部分需要根据模型自身知识扩写到300字以上，如“熟悉当前智能照明应用主流协议及标准”、“具有良好的沟通能力和团队合作精神”。
    请根据用户输入的信息生成JSON格式的招聘信息；其中岗位职责和任职要求的字数需要在400到500字数之间，只需要返回JSON数据即可，不需要进行解释；如果有分条叙述请保留序号，每条结尾使用“\n”标签进行换行。返回格式示例如下：
    {
        "job_title": "测试工程师",
...
def optimize_recruitment_system_content():
    ...
    7.岗位职责：变量名为“job_responsibilities”，指该职位需要完成的主要工作任务和责任范围，此部分需要根据模型自身知识扩写到300字以上，如“负责家居智能照明产品研发”、“配合项目主管完成项目的监控和统筹”等。
    8.任职要求：变量名为“job_requirements”，又称“岗位要求”，是指胜任该职位所需具备的能力、技能、经验及其他素质要求，此部分需要根据模型自身知识扩写到300字以上，如“熟悉当前智能照明应用主流协议及标准”、“具有良好的沟通能力和团队合作精神”。
    请根据用户输入的<优化要求></优化要求>标签中要求，优化<招聘信息></招聘信息>标签中的内容，然后生成JSON格式的招聘信息；其中岗位职责和任职要求的字数需要在400到500字数之间，只需要返回优化后的JSON数据即可，不需要进行解释；如果有分条叙述请保留序号，每条结尾使用“\n”标签进行换行。返回格式示例如下：
    {
        "job_title": "测试工程师",
        "salary": "8,000-12,000元/月",
        "work_experience": "三年或以上质量管理工作经验",
